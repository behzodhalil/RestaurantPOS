...
class FetchOrdersBetweenBySearchTextInteractorImpl @Inject constructor(
    private val orderRepository: OrderRepository,
    private val dispatcherProviders: DispatcherProviders
) : FetchOrdersBetweenBySearchTextInteractor {

    override fun invoke(searchText: String, start: Long, end: Long): Flow<List<FoodOrder>> {
        return orderRepository.fetchOrdersBetweenBySearchText(searchText,start,end).flowOn(dispatcherProviders.io)
    }
}