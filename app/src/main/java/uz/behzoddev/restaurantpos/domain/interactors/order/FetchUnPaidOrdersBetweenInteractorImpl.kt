...
class FetchUnPaidOrdersBetweenInteractorImpl @Inject constructor(
    private val orderRepository: OrderRepository,
    private val dispatcherProviders: DispatcherProviders
) : FetchUnPaidOrdersBetweenInteractor {
    override fun invoke(start: Long, end: Long): Flow<List<FoodOrder>> {
        return orderRepository.fetchUnPaidOrdersBetween(start, end).flowOn(dispatcherProviders.io)
    }
}